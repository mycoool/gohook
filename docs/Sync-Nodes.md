...
- **忽略规则语义**：参考 Syncthing 的 `.stignore` 语法（glob、`!` 反选、`#` 注释）。
  ...
  - 快速指南（Syncthing 风格子集）：
    ...
    | --- | --- |
    | `# comment` | 注释行 |
    | `!pattern` | 反选（取消忽略），按“最后匹配规则生效” |
    | `*` | 单层通配（不跨目录） |
    | `**` | 多层通配（可跨目录） |
    | `?` / `[abc]` | 单字符/字符组匹配 |
    | `dir/` | 目录规则：匹配该目录及其所有子路径 |
    | `/pattern` | 锚定到项目根（不加 `/` 时，且规则本身不含 `/`，默认视为 `**/pattern`） |
  - 行为说明：
    - 规则匹配使用 `/` 作为路径分隔符（Windows 也建议写 `/`）。
    - 默认区分大小写（当前不支持 Syncthing 的 `(?i)` 前缀）。
    - 当前不支持 Syncthing 的 `(?d)`、`//` 注释等扩展语法；请使用 `#` 注释。
  - 示例：
    - `node_modules/**`：忽略任意层级的 `node_modules`
    - `*.log`：忽略任意目录下的 `*.log`
    - `runtime/`：忽略 `runtime` 目录及其子路径（默认忽略列表的一部分）
    - `!.env`：取消忽略 `.env`（需要放在忽略规则之后才生效）
...
为避免启用增量索引后出现“目标端误删文件但主节点未变更 ⇒ 文件长期无法恢复”的情况，Agent 会在目标目录保存一份上次成功同步的文件清单：

- 文件：`.gohook-sync-expected.json`
- 行为：下次任务开始时，若发现清单中的文件在目标端缺失，会发送 `index_need` 请求，主节点会强制补发这些路径的索引条目，从而恢复文件。
