...
const MATCH_TYPES = [
    ...
    {value: 'payload-hmac-sha1', label: 'HMAC-SHA1签名', description: 'GitHub webhook签名验证'},
    {
        ...
        label: 'HMAC-SHA256签名',
        description: 'GitHub/GitLab webhook签名验证',
    },
...
export default function HookConfigDialog({open, onClose, hook, onSave}: HookConfigDialogProps) {
    ...
    const removeParameter = (index: number) => {
        const updatedParams = (formData['pass-arguments-to-command'] || []).filter(
            (_, i) => i !== index
        );
        handleFieldChange('pass-arguments-to-command', updatedParams);
    };
    ...
    const removeEnvironmentVariable = (index: number) => {
        const updatedEnvs = (formData['pass-environment-to-command'] || []).filter(
            (_, i) => i !== index
        );
        handleFieldChange('pass-environment-to-command', updatedEnvs);
    };
        ...
        <Grid container spacing={3}>
            ...
            <Grid size={12}>
                <TextField
                    ...
                    helperText={
                        errors['execute-command'] || '当webhook被触发时执行的命令或脚本路径'
                    }
                    ...
                    {ruleType === 'simple' ? (
                        <Box>
                            ...
                            <Grid container spacing={2}>
                                ...
                                {(simpleRule.type.includes('hmac') ||
                                    simpleRule.type === 'scalr-signature') && (
                                    <Grid size={12}>
                                        <TextField
                                            fullWidth
                                            label="密钥"
  ...
  "X-Custom-Header": "value"
    ...
    return (
        <Dialog
            ...
            <DialogTitle>{isEditMode ? '编辑Webhook配置' : '添加Webhook配置'}</DialogTitle>
            ...
            <DialogContent sx={{p: 0}}>
                <Box sx={{borderBottom: 1, borderColor: 'divider'}}>
                    <Tabs
                        ...
                        aria-label="hook configuration tabs">
